...
beforeEach(() => {
	let fs = require("fs");
	let fileContent = fs.readFileSync("appointments/templates/confirm_appointment_update.html");
	document.open();
...
describe("DOM tests", () => {
	...
	test("expects to find p tag text", () => {
		expect(document.getElementsByTagName("p")[0].innerHTML).toBe("Doctor: {{ appointment_to_update.FirstName }} {{ appointment_to_update.LastName }} ");
	});
	
	test("expects to find p tag text", () => {
		expect(document.getElementsByTagName("p")[1].innerHTML).toBe("On/At: {{ appointment_to_update.AppointmentDate }} {{ appointment_to_update.AppointmentTime }} ");
	});
	...
	test("expects to find action for the form", () => {
		expect(document.getElementsByTagName("form")[0].getAttribute("action", "{{ url_for('confirm_appointment_update', appointment=appointment_to_update.AppointmentId) }}"));
	});
	...
	test("expects to find button tag text", () => {
		expect(document.getElementsByTagName("button")[0].innerHTML).toBe("Update");
	});
