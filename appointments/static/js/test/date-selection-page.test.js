...
beforeEach(() => {
	let fs = require("fs");
	let fileContent = fs.readFileSync("appointments/templates/choose_date_time.html");
	document.open();
...
describe("DOM tests", () => {
	...
	test("expects to find action for the form", () => {
		expect(document.getElementsByTagName("form")[0].getAttribute("action", "{{ url_for('choose_date_time') }}"));
	});
	...
	test("expects to find options for the selects tags on the page", () => {
		expect(document.getElementsByTagName("option").length).toEqual(2);
	});
	...
	test("expects to find option tag value", () => {
		expect(document.getElementsByTagName("option")[1].getAttribute("value", "{{ atime.AvailableTimes }}"));
	});
	
	test("expects to find option tag text", () => {
		expect(document.getElementsByTagName("option")[1].innerHTML).toBe("{{ atime.AvailableTimes }}");
	});
